...
class Gallery extends Component {
  state = {selectedThumbnail: imagesList[0].id}
  ...
  render() {
    ...
    const {imageUrl, imageAltText} = imagesList[selectedThumbnail]
    return (
      <div className="main-container">
        <div className="gallery-container">
          <img src={imageUrl} alt={imageAltText} className="main-image" />
          <h1 className="heading">Nature Photography</h1>
          <p className="by-name">Nature Photography by Rahul</p>
          <ul className="thumbnails-main-list">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                key={eachImage.id}
                imageDetails={eachImage}
                clickThumbnailItem={this.clickThumbnailItem}
                isSelected={selectedThumbnail === eachImage.id}
              />
            ))}
          </ul>
        </div>
